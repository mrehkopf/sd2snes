...
void msu_savecheck(int immediate) {
  ...
  if(immediate || (getticks() > msu_last_sram_check + 100)) {
    currentcrc = calc_sram_crc(SRAM_SAVE_ADDR + romprops.srambase, romprops.sramsize_bytes);
...
/* it is assumed that the MSU file is already opened by calling msu1_check(). */
  ...
  msu_last_crc = calc_sram_crc(SRAM_SAVE_ADDR + romprops.srambase, romprops.sramsize_bytes);
